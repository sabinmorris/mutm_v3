	...
	session_start();//start session
  	...
  	try {
	  	...
	  	if (isset($_POST['uUpd'])) {
			...
			$useridU = inp($_POST['userid']);


			$eventts = 'Amebadilisha taarifa za mtumiaji ' . $userName; //get event
			$userid = $_SESSION['userid']; //get userid

			include ("UserClass.php"); //call class

			// CONVERTING USER INPUT TO OBJECT - JSON OBJECT 
			$myObj = new UserUpdateClass();
			...
			$api ='updateUsers?id='.$useridU.'&email='. $_SESSION['username'].'&token='.$_SESSION['logintoken']; //define API name
